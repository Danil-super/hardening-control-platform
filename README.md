...
1. В панели откройте «Хосты» → «Добавить хост», заполните адрес и пользователя SSH. Если доступ платформе уже выдан, сразу нажмите «Проверить подключение».
2. Сведения о доступе находятся в раскрываемом разделе «Доступ по SSH». «Ключ платформы» содержит публичный ключ управляющего узла. Если доступ ещё не выдан, администратор целевой VM должен разрешить вход этому ключу через существующую систему управления или консоль. Повторная настройка уже доступного хоста не нужна.
...
4. В разделе «Ключ сервера» вставьте подтверждённый fingerprint и сохраните проверенный ключ. Выполните «Проверить подключение», затем «Добавить хост».
...
«Обновить сведения» повторно читает существующий SSH-ключ платформы и показывает уведомление. Кнопка не заменяет ключ. После входа открывается страница хостов (или страница, ради которой потребовался вход). Результаты действий показываются во всплывающих уведомлениях; ошибки остаются до закрытия. При запрете автоматического копирования можно выделить содержимое поля и скопировать вручную.
...
Пакетная интеграция Trivy обрабатывает системные dpkg-пакеты Debian/Ubuntu; полнота зависит от инвентаря, поддерживаемого выпуска и базы. Для CVE Astra используется отдельный OVAL-аудит. RPM-результаты помечаются частичными; библиотеки приложений, вручную установленное ПО и контейнерные образы требуют отдельного SBOM.
