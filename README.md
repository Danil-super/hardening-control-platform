...
В рабочем режиме откройте «Политики» и назначьте проверенный datastream и точный профиль для отдельной inventory-группы, например `scap_hosts`. При запуске OpenSCAP HCP передаёт выбор только для этой группы, фиксирует профиль, checksum и mtime datastream в отчёте. Для конкретного правила можно добавить временное согласованное исключение с причиной и датой окончания: оно остаётся видимым как `manual`, а не скрывает находку.

Переменные `.env` остаются fallback для systemd-планировщика или групп без записи в «Политиках»:
