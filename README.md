...
В панели «Источники» сначала выберите источник CVE-данных Trivy:

- **Сетевая база** — нажмите «Сохранить и загрузить»; HCP скачает общую CVE-базу через публичный registry либо заданное внутреннее OCI-зеркало. При уже выбранном режиме используйте «Загрузить общую базу CVE», затем — «Обновить общую базу CVE».
...
Trivy использует **одну общую базу CVE** для пакетов поддерживаемых Linux-систем, а не отдельную базу для каждого хоста или выпуска ОС. HCP сохраняет её на control node; после успешной загрузки обновляйте её кнопкой «Обновить общую базу CVE». Сам аудит использует уже подтверждённый снимок базы и не зависит от кратковременной недоступности registry.

Убедитесь, что блок «Свежесть базы Trivy» показывает «актуальна», затем на странице «Хосты» раскройте «Дополнительные проверки», выберите «Пакеты и CVE — Trivy» и запустите проверку. HCP сохранит инвентарь, сформирует CycloneDX SBOM, запустит Trivy и покажет установленную/фиксированную версию, статус поставщика и источник данных. Если базы нет, она старше лимита или дату нельзя прочитать, CVE-отчёт помечается частичным: отсутствие CVE в таком отчёте не считается доказательством безопасности.
