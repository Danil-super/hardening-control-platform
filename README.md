...
Следуйте [единой инструкции Ubuntu → Astra, раздел 4](docs/ubuntu-astra-setup.md#host-onboarding). В ней указаны место выполнения каждой команды, ожидаемый результат и действия при ошибке. Другого SSH-ключа для этого маршрута создавать не нужно.

Порядок: **учётная запись и SSH → разрешённый ключ платформы → выбранный режим прав → подтверждённый ключ сервера → проверка подключения → сохранение хоста**. При уже подготовленном доступе переходите к проверке через сайт.

Если хост добавился только без sudo, используйте [настройку sudo](docs/ubuntu-astra-setup.md#sudo-access), затем [обновление существующего хоста](docs/ubuntu-astra-setup.md#existing-host). Галочка «Использовать sudo» использует уже выданные права и не выдаёт их автоматически. Работа без sudo от обычного пользователя ограничена его разрешениями; она не подтверждает готовность к полному аудиту и изменениям firewall.

На странице «Хосты» блоки идут в порядке: сканирование сети → первое подключение по SSH → форма добавления. «Перечитать ключ» только загружает существующий общий ключ платформы; порядок его замены доступен по отдельной ссылке. Новые хосты по умолчанию используют sudo. Уведомления исчезают через 3 секунды (при наведении или фокусе таймер приостановлен). После успешного входа открывается страница хостов; результаты действий появляются в уведомлениях. Для CVE Astra используется отдельный OVAL-аудит по базе производителя, а не вывод о полном покрытии на основании Trivy.
