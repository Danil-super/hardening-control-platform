...
Следуйте [единой инструкции Ubuntu → Astra, раздел 4](docs/ubuntu-astra-setup.md#host-onboarding): **сканирование → выбор хоста → подтверждение сервера → пароль SSH → «Создать ключ и настроить доступ» → проверка → «Добавить хост»**. Для каждой машины создаётся своя пара; команда ручного добавления общего ключа больше не требуется.
