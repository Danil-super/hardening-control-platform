...
«Обновить сведения» только перечитывает существующий ключ платформы. После успешного входа открывается страница хостов; результаты действий появляются в уведомлениях. Для CVE Astra используется отдельный OVAL-аудит по базе производителя, а не вывод о полном покрытии на основании Trivy.
