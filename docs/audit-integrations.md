...
- **Локальная база** — Trivy запускается с `--offline-scan --skip-db-update`; сеть и registry не опрашиваются, используется только существующий cache в persistent volume HCP.
...
Если public Internet запрещён, но есть внутреннее OCI-зеркало, укажите его и оставьте **сетевой режим**: control node будет обращаться только к этому зеркалу.
...
Для полностью изолированной сети сначала подготовьте и проверьте cache Trivy в отдельное утверждённое окно обновления, затем в панели выберите **Локальная база**. `offline` запрещает обновление и любой сетевой запрос. Если локальной базы нет, HCP создаёт запись `manual` «Trivy не выполнил CVE-сопоставление». Отсутствие CVE тогда не считается результатом проверки. Актуальность cache — отдельный объект контроля: фиксируйте дату его загрузки и повторяйте обновление по регламенту.
