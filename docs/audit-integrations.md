...
Проверьте точный идентификатор профиля через `oscap info`, затем в разделе HCP «Политики» задайте соответствие inventory-группы, datastream и профиля. Для группы можно добавить временное исключение только по конкретному ID правила: причина и дата окончания обязательны, а результат остаётся в отчёте как `manual` с evidence исключения. Если хост попадает в несколько групп с разными профилями, запуск блокируется до устранения конфликта.

Переменные ниже остаются fallback для systemd-планировщика и для групп без политики в интерфейсе:
