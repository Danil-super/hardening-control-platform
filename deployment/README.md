...
Для каждого нового хоста используйте [парольную настройку через сайт](../docs/ubuntu-astra-setup.md#host-onboarding): укажите пользователя и пароль Astra, нажмите «Подключить хост». При первом входе подтвердите сервер по запросу формы. Вход по паролю, создание пары, установка публичного ключа, однократная настройка `sudo` для пользователя не root, проверка и сохранение выполняются платформой. HCP создаёт отдельную пару для каждой машины; приватные файлы хранятся в `/var/lib/hcp/ssh-host-keys` с правами `600`. На целевой хост передаётся только публичная часть. Пароль не сохраняется; созданное HCP правило sudoers удаляется при завершении работ по хосту. Для этого используется системный `/usr/bin/python3` с Paramiko на управляющей машине; Docker-образ содержит зависимость.
...
Для сетевого режима откройте «Источники», выберите «Сетевая база» и нажмите «Сохранить и загрузить»; HCP сам заполнит `/var/lib/hcp/trivy-cache` и подтвердит свежесть по `metadata.json`. Это одна общая база CVE Trivy для поддерживаемых ОС, не отдельная загрузка на каждый хост. После первой загрузки используйте кнопку «Обновить общую базу CVE»; сам аудит работает с уже подтверждённым снимком. Если `HCP_TRIVY_DB_REPOSITORY` не задан, HCP применяет официальные резервные источники Trivy. Для полностью изолированного режима сначала подготовьте этот cache в утверждённое окно обновления, затем выберите «Локальная база» в интерфейсе. Один адрес зеркала сам по себе не заполняет cache: в offline-режиме Trivy не обновляет базы. Если cache отсутствует или недоступен, HCP создаёт неполный отчёт с ручной проверкой. Возраст базы контролируется по `HCP_TRIVY_MAX_DB_AGE_HOURS`; при превышении лимита отчёт тоже помечается частичным. Переключатель относится к Trivy, а не ко всем интеграциям и обновлениям ОС.
