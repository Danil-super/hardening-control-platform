...
const additionalAuditActions = [
  ...
  },
  {
    ...
    group: "Диагностика",
    description: "Проверяет TCP-порты выбранного хоста с управляющей машины. Перед запуском выберите глубину проверки.",
    icon: Search,
    requiresConfirmation: true,
  },
  {
...
const nmapScanScopeOptions = [
  {
    id: "top_100",
    label: "Быстрая: 100 TCP-портов",
    description: "Короткая проверка наиболее распространённых сервисов. Не подтверждает отсутствие портов вне этой выборки.",
  },
  {
    id: "top_1000",
    label: "Стандартная: 1 000 TCP-портов",
    description: "Более широкий обзор распространённых сервисов; обычно занимает несколько минут.",
  },
  {
    id: "full_tcp",
    label: "Полная TCP: 1–65 535",
    description: "Проверяет весь TCP-диапазон. На фильтрующем firewall может идти до 30 минут; UDP сюда не входит.",
  },
] as const;
type NmapScanScope = (typeof nmapScanScopeOptions)[number]["id"];
...
export function AnsibleControlClient() {
  ...
  const [nmapScanScope, setNmapScanScope] = useState<NmapScanScope>("top_100");
  ...
  async function runAction(action: string, mode: "preview" | "apply" = "apply") {
    ...
    const confirmationText = action === "lynisTemporaryAudit"
      ...
      : action === "networkPortScan"
        ? nmapScanScope === "full_tcp"
          ? "Nmap проверит все TCP-порты 1–65535 выбранного хоста с control node. Это может занять до 30 минут и создаст много сетевых соединений. UDP не проверяется. Продолжить?"
          : `Nmap выполнит ${nmapScanScope === "top_1000" ? "стандартную проверку 1000" : "быструю проверку 100"} TCP-портов выбранного хоста с control node. Продолжить?`
        : "Запустить проверку?";
  ...
  const selectedNmapScanScope = nmapScanScopeOptions.find((scope) => scope.id === nmapScanScope) ?? nmapScanScopeOptions[0];
          ...
          <details className="mt-4 rounded-md border border-slate-800 bg-slate-900/70 p-3">
            ...
            <div className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,360px)_minmax(0,250px)_minmax(0,1fr)_auto] xl:items-end">
              ...
              {additionalAuditActionId === "networkPortScan" ? (
                <label className="block min-w-0">
                  <span className="text-xs font-semibold uppercase text-slate-500">Глубина Nmap</span>
                  <select
                    value={nmapScanScope}
