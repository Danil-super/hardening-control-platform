...
const setupSteps = [
  ...
  "Разрешённый ключ платформы. Публичный ключ из блока «Первое подключение по SSH» должен быть разрешён выбранному пользователю на Astra. Уже выданный доступ повторно не настраивается.",
  ...
  "Подтверждённый ключ сервера. Сверьте отпечаток через доверенную консоль или реестр и сохраните его в форме «Добавление хоста» → «Подтверждение сервера».",
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-slate-400">На странице «Хосты» сначала идёт сканирование сети, затем подготовка SSH и sudo, после неё — форма добавления. Все команды с местом выполнения и ожидаемым результатом собраны в основной инструкции.</p>
        </div>
      ...
      <section id="host-onboarding" className="grid scroll-mt-6 gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="h-fit space-y-5 rounded-md border border-slate-800 bg-slate-950/70 p-5">
          ...
          <p className="text-sm leading-6 text-slate-400">Используйте общий ключ из блока «Первое подключение по SSH». Он создан при установке в secrets/hcp-control. Новая команда ssh-keygen для каждого хоста не нужна.</p>
