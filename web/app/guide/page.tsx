...
const setupSteps = [
  "Найдите машину. Просканируйте свою подсеть или укажите адрес вручную. Заполните имя записи, пользователя Astra и порт SSH.",
  "Подтвердите сервер. Сверьте отпечаток через доверенную консоль или реестр и нажмите «Подтвердить сервер» в форме подключения.",
  "Введите пароль SSH. Откройте панель через HTTPS либо localhost. При наличии административных прав можно явно включить настройку беспарольного sudo.",
  "Нажмите «Создать ключ и настроить доступ». HCP создаст отдельную пару, установит публичный ключ и проверит новое подключение по нему. Пароль не сохранится.",
  "Проверьте результат. SSH, Python и выбранный режим sudo проверяются автоматически; повторить проверку можно кнопкой «Проверить подключение».",
  "Нажмите «Добавить хост». Затем выберите профиль и запустите аудит. Неполнота проверки и найденные нарушения отображаются отдельно.",
];
    ...
    <div className="space-y-6">
      ...
      <section id="host-onboarding" className="grid scroll-mt-6 gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="h-fit space-y-5 rounded-md border border-slate-800 bg-slate-950/70 p-5">
          ...
          <p className="text-sm leading-6 text-slate-400">Для каждой машины HCP создаёт свою пару. Приватный ключ остаётся на Ubuntu; на Astra передаётся только публичный. Перезапуск и повторное нажатие не меняют пару.</p>
      ...
      <section id="sudo-access" className="scroll-mt-6 rounded-xl border border-sky-400/20 bg-sky-400/5 p-5">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-300">После настройки включите «Использовать sudo», проверьте подключение и нажмите «Сохранить изменения». HCP проверяет настоящий запуск модуля Ansible с правами root. Если учётной записи sudo запрещено, права выдаёт администратор Astra.</p>
